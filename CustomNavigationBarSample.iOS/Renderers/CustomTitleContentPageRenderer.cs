...
[assembly: ExportRenderer(typeof(ContentPage), typeof(CustomTitleContentPageRenderer))]

namespace CustomNavigationBarSample.iOS.Renderers
{
   public class CustomTitleContentPageRenderer : PageRenderer
    {
        UILabel titleLabel;
            ...
            if (NavigationController != null && titleView != null)
            {
                ...
                if (!string.IsNullOrEmpty(CustomNavigationPage.GetTitleBackground(Element)))
                {
                    try
                    {
                        ...
                        marginView.Frame = titleView.Frame;
                    }
                    catch (Exception ex)
                    {
